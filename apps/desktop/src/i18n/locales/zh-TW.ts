...
export default withEnglishFallback({
  ...
  grid: {
    ...
    allLoaded: "已全部載入",
    fetchAllRows: "載入全部資料列",
    fetchAllRowsStop: "停止載入",
    fetchAllRowsProgress: "已載入 {loaded} / 共 {total} 列",
    fetchAllRowsProgressUnknown: "已載入 {loaded} 列",
    fetchAllRowsConfirmMessage: "將把該查詢的全部資料列載入到結果表格並保留在記憶體中（共 {total} 列）。",
    fetchAllRowsConfirmLarge: "該結果集共 {total} 列，超過 {threshold} 列。全部載入會佔用大量記憶體，可能導致介面卡頓。",
    fetchAllRowsConfirmUnknown: "無法預先確定總列數，將持續載入直到讀完或達到上限 {cap} 列。",
    fetchAllRowsConfirmDetails: "最多載入 {cap} 列，可在「設定 → 資料 → 查詢結果最大列數」中調整。",
    fetchAllRowsSuppress: "以後載入全部資料列不再提示",
    fetchAllRowsLimitReached: "已達到查詢結果最大列數（{cap} 列），可在「設定 → 資料 → 查詢結果最大列數」中調整。",
